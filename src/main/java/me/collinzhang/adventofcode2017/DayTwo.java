...
public class DayTwo {
    ...
    public static void main(String[] args) {
        System.out.println(dayTwo1(DAY_TWO));
        System.out.println(dayTwo2(DAY_TWO));
    }

    private static long dayTwo1(String input) {
    ...
    private static long dayTwo2(String input) {
        Scanner scanner = new Scanner(input);
        long sum = 0;
        while (scanner.hasNext()) {
            String nextLine = scanner.nextLine();
            String[] numbers = nextLine.split("\\s+");
            List<Integer> largeToSmall = Arrays.stream(numbers).map(Integer::parseInt).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
            List<Integer> smallToLarge = new ArrayList<>(largeToSmall);
            smallToLarge.sort(Comparator.naturalOrder());

            for (Integer i : largeToSmall) {
                for (Integer s : smallToLarge) {
                    if (i % s == 0 && i != s) {
                        sum += i / s;
                        break;
                    }
